...
void createTotalComponents(const string components[], int components_max[],
                           int component_max_grades[], int size,
                           vector<string> &component_arrays) {
  ...
  for (int i = 0; i < size; i++) {
    for (int o = 1; o <= components_max[i]; o++) {
      ...
      component_arrays.push_back(components[i] + "_" + to_string(o) + "," +
                                 to_string(max_grades) + "," +
                                 to_string(genRandomVal(0, max_grades)));
    }
  }
}

// combines no_of_components with teachers
void combineComponentswithSubjects(string subjects[], string subject_code[],
                                   int credit_hours[], int size,
                                   vector<string> all_components,
                                   vector<string> &subject_with_components) {
  string line;
  for (int i = 0; i < size; i++) {
    // oop,CS00,4
    line = subjects[i] + "," + subject_code[i] + "," +
           to_string(
               credit_hours[i]); // creates subject with its code, credit hours
    string combine;
    // oop,
    for (int o = 0; o < all_components.size(); o++) {
      combine = line + "," + all_components[o];
      subject_with_components.push_back(
          combine); // now it gets each component for each subject
    }
  }
}
...
int main() {
  ...
  string subjects[5] = {"oop", "embedded-systems", "data-communications",
                        "operating-systems", "applied-maths-ii"};
  ...
  vector<string> all_components;
  createTotalComponents(components, components_max, component_max_grades, 5,
                        all_components);
  vector<string> subject_with_components;
  combineComponentswithSubjects(subjects, subject_code, subject_credit_hours, 5,
                                all_components, subject_with_components);
  ...
  */
  int n_of_students = 5;
  wf << "#id,name,semester,subject,subject_code,credit_hours,component_type_n,"
        "total,obtained(random)"
     << endl;

  for (int student = 1; student <= n_of_students; student++) {

    for (int i = 0; i < subject_with_components.size(); i++) {
      wf << "7017707" << student << "," << "Name" << student << ","
         << "Fall-2025" << "," << subject_with_components[i] << endl;
    }
  }
}